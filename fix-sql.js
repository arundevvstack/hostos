const fs = require('fs');
const content = fs.readFileSync('supabase/migrations/apply_all_remaining.sql', 'utf8');
const idx = content.indexOf('create index if not exists idx_dlq_user on public.dead_letter_queue(user_id);');

if (idx !== -1) {
  const cleanContent = content.substring(0, idx + 77) + `
...
`;
  ...
  fs.writeFileSync('supabase/migrations/apply_all_remaining.sql', cleanContent);
